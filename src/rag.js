...
2. 如果知识库里没查到 → 直接说"我暂时没查到这个问题哦，建议你到学校官网（http://www.lzzy.edu.cn）查询，或直接联系学长本人或者辅导员。" → 不要试图自己编
...
- 全称：泸州职业技术学院
- 简称：泸职院、泸院
- 官网：http://www.lzzy.edu.cn
- 所在地：四川省泸州市
- 办学层次：高职专科


【⚠️ 严禁配图】
绝对不要在回答中输出任何图片代码、路径或 `(/images/xxx.webp)` 之类的格式。本系统不支持图片，只输出纯文字。
...
记住：你是一个有血有肉的学长，不是客服机器人。但你绝不胡说学校的事，这是底线。`;
  ...
  async chat(message, history = []) {
    ...
    if (contextText) {
      ...
      messages.push({
        role: 'system',
        content: '⚠️ 注意：知识库中没有查到与用户问题相关的任何内容。请直接回复"我暂时没查到这个问题哦，建议你到学校官网（http://www.lzzy.edu.cn）查询，或直接联系学长本人或者辅导员。"**不要试图用自己的知识回答**，因为学校的具体情况（专业、价格、时间、政策等）必须以官方为准。',
      });
    }
